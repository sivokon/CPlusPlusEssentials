...
int main()
{
	...
	for (int i = 0; i < sentence.length(); i++) {
		...
		if (sentence[i] == '*') {
			if (asteriks == 1) {
				//переводим слово в верхний регистр или оно в нижнем, удаляем звездочки по бокам, начинаем считывать слово заново
		...
		if (sentence[i] == '_') {
			if (underline == 1) {
				//вставляем пробелы между буквами, меняем подчеркивания на пробелы по бокам, начинаем считывать слово заново
				int counter = 1;
				for (int j = 0; j < word.length(); j++) {
